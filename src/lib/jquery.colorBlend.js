...
(function(jQuery) {
	var ver = '3.0.3', actionWords = ["play", "start", "stop", "pause", "resume", "reverse", "reset"], cb = [];
	...
	function SetOptions(opts) {
		if (!opts.internals.animating) {
			...
			var interval = Math.floor(1000 / opts.fps), totalFrames = Math.floor(opts.duration / interval);
	...
	function GetAlpha(opts) {
		var modifier = ((opts.alpha.length - 1) * opts.internals.pct()), frames = opts.internals.frames, step = Math.floor(modifier), percent = (modifier - step), curFrame = (frames * percent), fc = opts.alpha[step], tc = opts.alpha[step + 1], result = 0;
		if (tc > -1 && tc < 101 && fc > -1 && fc < 101) {
			result = Math.floor(fc * ((frames - curFrame) / frames) + tc * (curFrame / frames)) * .01;
		}
		return result;
	}
