...
//		AP.statusMsg.show('');
	...
	getAttr : function(node, attr) {
		...
		/*if (!ret && node.getAttributeNS && attr.match(/:/)) {	// attr.match(/:/)の所でバグ発生のためコメントアウト
			var prfx = attr.split(':')[0];
...
Event.observe( window,'load', AP.launch );	// prototype.js使っている場合はこの方が安全
